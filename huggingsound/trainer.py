...
class CTCTrainer(Trainer):
    ...
    def __init__(self, min_learning_rate=None, lr_warmup_steps=None, lr_decay_steps=None, use_cuda_amp:bool=False,
...
def del_key(key:str, target_dict:dict):
...
def finetune_ctc(model, model_name_or_path: str, output_dir: str, processor: Wav2Vec2Processor, train_dataset: Dataset, 
    ...
    unsupported_keys = ['device', '_n_gpu', 'batch_creation_callbacks', 'early_stopping_patience', 'evaluation_callbacks', 'ignore_pretrained_weights', 'lr_decay_steps', 'lr_warmup_steps', 'metrics_batch_size', 'min_learning_rate', 'pad_to_multiple_of', 'show_dataset_stats', 'training_step_callbacks', 'use_8bit_optimizer']
    for i, unsupported_key in enumerate(unsupported_keys):
        del_key(unsupported_key, training_args_dict)
